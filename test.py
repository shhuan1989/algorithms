...
class SHA256(object):
    ...
    def hexdigest(self):
        ...
        for idx, shift in zip(range(56, 64), [v for v in range(24, -1, -8)] * 2):
...
def test():
    ...
    #
    # assert hashlib.sha256(string.encode('utf-8')).hexdigest() == SHA256(string).hexdigest()
    #
    ...
    #
    # print(hashlib.sha256(str('hello').encode('utf-8')).hexdigest(), SHA256('hello').hexdigest())
    assert hashlib.sha256(str('hello⊕').encode()).hexdigest() == SHA256('hello⊕').hexdigest()
