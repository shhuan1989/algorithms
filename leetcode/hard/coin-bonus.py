...
"""
created by shhuan at 2019/12/8 00:12
...
class SegmentTree:
    def __init__(self, N):
        self.data = [0 for _ in range(4*N+1)]
        self.lazy = [0 for _ in range(4*N+1)]
    ...
    def update(self, node, nodel, noder, targetl, targetr, add):
        if targetl > targetr or noder < targetl or nodel > targetr:
            return
        ...
        if nodel == targetl and noder == targetr:
            ...
            self.data[node] += (noder-nodel+1) * self.lazy[node] + (targetr - targetl + 1) * add
            self.data[node] %= MOD
            self.push(node)
            nodem = (nodel + noder) // 2
            self.update(node * 2, nodel, nodem, targetl, min(targetr, nodem), add)
            self.update(node * 2 + 1, nodem+1, noder, max(targetl, nodem+1), targetr, add)

    def sum(self, node, nodel, noder, targetl, targetr):
        if targetr < targetl or noder < targetl or nodel > targetr:
            return 0
        if nodel == targetl and noder == targetr:
            return (self.data[node] + self.lazy[node] * (targetr-targetl+1)) % MOD

        self.data[node] += self.lazy[node] * (noder-nodel+1)
        self.push(node)
        nodem = (nodel + noder) // 2

        return (self.sum(node*2, nodel, nodem, targetl, min(targetr, nodem)) +
                self.sum(node*2+1, nodem+1, noder, max(targetl, nodem+1), targetr)) % MOD
