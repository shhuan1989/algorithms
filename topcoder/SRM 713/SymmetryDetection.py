...
def do_test(board, __expected):
    startTime = time.time()
    instance = SymmetryDetection()
    exception = None
    try:
        __result = instance.detect(board);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("SymmetryDetection (250 Points)\n\n")
    ...
    with open("SymmetryDetection.sample", "r") as f:
        while True:
            label = f.readline()
