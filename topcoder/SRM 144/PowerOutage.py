...
class PowerOutage:
    def estimateTimeOut(self, fromJunction, toJunction, ductLength):
        if not fromJunction or not toJunction or not ductLength:
            return 0

        links = collections.defaultdict(list)
        for i in range(len(fromJunction)):
            links[fromJunction[i]].append((toJunction[i], ductLength[i]))

        totalLen = 2 * self.dfs(0, links, 0)
        mxl = self.maxLen(0, links)
        # print(totalLen, mxl)

        return totalLen - mxl

    def dfs(self, root, links, count):
        if root not in links:
            return count
        res = count
        for k, v in links[root]:
            res += self.dfs(k, links, v)
        return res

    def maxLen(self, root, links):
        if root not in links:
            return 0
        res = 0
        for k, v in links[root]:
            res = max(res, self.maxLen(k, links)+v)
        return res
