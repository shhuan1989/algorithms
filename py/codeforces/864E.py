...
先按照物品烧毁时间，物品价值，抢救时间，物品编号排序，并存在D,P,T,I中。
然后使用动态规划计算时间t内抢救前i个物品能够挽回的最大损失。


dp[t][i] = max(dp[t][i-1], dp[t-T[i]][i-1] + P[i])，其中T[i] <= t < D[i]。

如果dp[t][i] == dp[t-T[i]][i-1] + P[i]，那么表示抢救了物品编号为I[i]的物品。
