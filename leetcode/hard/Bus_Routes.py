...
"""
created by shhuan at 2018/12/18 00:03
...
class Solution:
    def numBusesToDestination(self, routes, S, T):
        ...
        busAtStation = collections.defaultdict(list)
        stations = set()
        for bus, route in enumerate(routes):
            for station in route:
                stations.add(station)
                busAtStation[station].append(bus)
        ...
        for bus, route in enumerate(routes):
            ...
            connection[route[-1]] = route[0]
            routesForBus[bus] = connection

        leastBusToStation = {(bus, S): 1 for bus in range(len(routes)) if S in routes[bus]}
        q = [(1, S, bus) for bus in busAtStation[S]]
        vis = {(S, bus) for bus in busAtStation[S]}
        ans = float('inf')
        heapq.heapify(q)
        while q:
            change, station, bus = heapq.heappop(q)
            if station == T:
                ans = min(ans, change)

            for nextBus in busAtStation[station]:
                if nextBus == bus:
                    if (routesForBus[bus][station], bus) not in vis:
                        vis.add((routesForBus[bus][station], bus))
                        heapq.heappush(q, (change, routesForBus[bus][station], bus))
                else:
                    if (nextBus, station) not in leastBusToStation or leastBusToStation[(nextBus, station)] > change + 1:
                        leastBusToStation[(nextBus, station)] = change + 1
                        heapq.heappush(q, (change + 1, station, nextBus))
